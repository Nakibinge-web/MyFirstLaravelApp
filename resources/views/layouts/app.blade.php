...
<body class="bg-gray-100">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-gray-900 text-white transform -translate-x-full transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                ...
                <div class="pt-4 mt-4 border-t border-gray-700">
                    ...
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-800 transition-colors {{ request()->routeIs('admin.*') ? 'bg-red-600 hover:bg-red-700' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if(session('budget_notification'))
    <script>
        // Show toast notification for budget alert
        document.addEventListener('DOMContentLoaded', function() {
            const notificationData = @json(session('budget_notification'));
            if (notificationData && typeof showNotification === 'function') {
                const icon = notificationData.type === 'exceeded' ? '⚠️' : '⚡';
                showNotification(
